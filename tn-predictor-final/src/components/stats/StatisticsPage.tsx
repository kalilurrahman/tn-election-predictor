...
type FullPoliticalHistoryPayload = {
  state: string;
  scope_note?: string;
  source_note?: string;
  election_records: FullHistoryElectionRow[];
  cm_terms: FullHistoryCMTerm[];
};

type CMTenureSummaryRow = {
  name: string;
  party: string;
  terms: number;
  total_days: number;
...
const diffDays = (start: Date, end: Date): number => {
  ...
  if (delta < 0) return 0;
  return Math.floor(delta / DAY_MS) + 1;
};
...
const formatTurnout = (value: number | null): string => {
  if (typeof value !== 'number') return 'NA';
  return `${value.toFixed(2)}%`;
};

const summarizeCMTenure = (terms: FullHistoryCMTerm[]): CMTenureSummaryRow[] => {
  const now = new Date();
  const bucket = new Map<string, CMTenureSummaryRow>();

  for (const term of terms) {
    const start = parseISODate(term.start_date);
    const end = parseISODate(term.end_date) ?? now;
    if (!start || end < start) continue;

    const key = `${term.name}||${term.party}`;
    const existing = bucket.get(key);
    const days = diffDays(start, end);
    const endLabel = term.end_date ?? now.toISOString().slice(0, 10);
    ...
    if (!existing) {
      bucket.set(key, {
        ...
        terms: 1,
        total_days: days,
        total_years: Number((days / 365.25).toFixed(2)),
        first_start: term.start_date,
        last_end: endLabel,
        acting_only: Boolean(term.is_acting),
      });
      continue;
    }

    const totalDays = existing.total_days + days;
    bucket.set(key, {
      ...existing,
      terms: existing.terms + 1,
      total_days: totalDays,
      total_years: Number((totalDays / 365.25).toFixed(2)),
      first_start: term.start_date < existing.first_start ? term.start_date : existing.first_start,
      last_end: endLabel > existing.last_end ? endLabel : existing.last_end,
      acting_only: existing.acting_only && Boolean(term.is_acting),
    });
  }

  return [...bucket.values()].sort((a, b) => b.total_days - a.total_days);
};
...
export const StatisticsPage = () => {
  ...
  const [fullHistory, setFullHistory] = useState<FullPoliticalHistoryPayload | null>(null);

  const avgRecentTurnout =
  ...
  const sortedElectionRows = [...(fullHistory?.election_records ?? [])].sort((a, b) => a.year - b.year);
  const turnoutRows = sortedElectionRows.filter((row) => typeof row.turnout_percent === 'number');
  const avgTurnoutAllCycles = turnoutRows.length
    ? Number((turnoutRows.reduce((sum, row) => sum + (row.turnout_percent ?? 0), 0) / turnoutRows.length).toFixed(2))
    : avgRecentTurnout;
  const firstTracked = sortedElectionRows[0];
  const latestTracked = sortedElectionRows[sortedElectionRows.length - 1];
  const electionsCoveredLabel = firstTracked && latestTracked ? `${firstTracked.year}-${latestTracked.year}` : '2011-2026*';
  const turnoutTimeline = turnoutRows.map((row) => ({ year: String(row.year), turnout: Number(row.turnout_percent ?? 0) }));
  const seatSizeTimeline = sortedElectionRows.map((row) => ({
    year: String(row.year),
    seats: row.seats_total,
    turnout: row.turnout_percent,
  }));

  const tenureRows = summarizeCMTenure(fullHistory?.cm_terms ?? []);
  const cmRowsOnly = tenureRows.filter((row) => row.name !== "President's Rule");
  const topCmRows = cmRowsOnly.slice(0, 10);
  const topCmChartRows = topCmRows.map((row) => ({
    name: row.name.length > 16 ? `${row.name.slice(0, 16)}...` : row.name,
    years: Number(row.total_years.toFixed(1)),
    terms: row.terms,
  }));
  const presidentsRule = tenureRows.find((row) => row.name === "President's Rule");
  const longestServingCM = cmRowsOnly[0];
  const uniqueCMs = new Set(cmRowsOnly.map((row) => row.name)).size;
  const currentlyServing = cmRowsOnly.find((row) => row.last_end === new Date().toISOString().slice(0, 10));
  ...
  useEffect(() => {
    ...
    fetch('/tn_full_political_history.json')
      .then((res) => (res.ok ? res.json() : null))
      .then((json) => setFullHistory(json))
      .catch(() => setFullHistory(null));
  }, []);
  ...
  const swingWaveData = (() => {
    ...
    for (const row of all) {
      const value = row.vote_swing_pct;
      const hit = bins.find((bucket) => value >= bucket.min && value < bucket.max);
      if (hit) hit.seats += 1;
    }
    return bins;
  })();
    ...
    <main className="flex-1 container mx-auto px-4 md:px-8 py-8 h-full">
      ...
      >
        ...
        <p className="text-sm md:text-[15px] text-muted-foreground mt-2 leading-relaxed">
          Historical records, vote-share movement, turnout behavior, and constituency-level intelligence for scenario planning.
        </p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5">
          <StatCard title="Elections Covered" value={electionsCoveredLabel} subtitle={`${sortedElectionRows.length || 3} assembly cycles`} />
          <StatCard title="Total Seats" value="234" subtitle="Tamil Nadu Assembly" />
          <StatCard title="Avg Turnout" value={`${avgTurnoutAllCycles}%`} subtitle={turnoutRows.length ? `Across ${turnoutRows.length} cycles` : 'Across 3 cycles'} />
          <StatCard title="Current Mode" value="Forecast + Archive" subtitle="Historical + simulation" />
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
          <StatCard title="Avg Winner Seats" value={`${avgWinnerSeats}`} subtitle="Recent-cycle baseline" />
          <StatCard title="Volatility Index" value={`${volatilityIndex}`} subtitle="Seat swing pressure" />
          <StatCard
            title="Longest Serving CM"
            value={longestServingCM ? `${longestServingCM.total_years.toFixed(1)}y` : 'NA'}
            subtitle={longestServingCM ? longestServingCM.name : 'Awaiting history data'}
          />
          <StatCard title="Unique CMs" value={`${uniqueCMs || 'NA'}`} subtitle="Post-independence count" />
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mt-6">
        ...
        <ChartPanel title="Turnout Trajectory (Recent Cycles)">
          <div className="h-[260px] sm:h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
      ...
      {turnoutTimeline.length > 0 && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mt-6">
          <ChartPanel title="Turnout Since Independence (Assembly Elections)">
            <div className="h-[280px] sm:h-[320px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={turnoutTimeline}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="year" />
                  <YAxis domain={[40, 85]} />
                  <Tooltip />
                  <Line type="monotone" dataKey="turnout" name="Turnout %" stroke="#0ea5e9" strokeWidth={3} dot />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </ChartPanel>

          <ChartPanel title="Chief Minister Tenure Length (Top 10)">
            <div className="h-[280px] sm:h-[320px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topCmChartRows}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="name" interval={0} angle={-24} textAnchor="end" height={70} />
                  <YAxis />
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="years" name="Years Served" fill="#7c3aed" radius={[8, 8, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </ChartPanel>
      ...
      {seatSizeTimeline.length > 0 && (
        <div className="grid grid-cols-1 mt-6">
          <ChartPanel title="Assembly Size and Turnout Evolution">
            <div className="h-[300px] sm:h-[340px]">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={seatSizeTimeline}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="year" />
                  <YAxis yAxisId="left" domain={[180, 390]} />
                  <YAxis yAxisId="right" orientation="right" domain={[40, 85]} />
                  <Tooltip />
                  <Legend />
                  <Bar yAxisId="left" dataKey="seats" name="Total Seats" fill="#1f2937" radius={[6, 6, 0, 0]} />
                  <Line yAxisId="right" type="monotone" dataKey="turnout" name="Turnout %" stroke="#0ea5e9" strokeWidth={3} dot />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </ChartPanel>
      ...
      <motion.div
        ...
        className="glass-panel rounded-3xl p-5 border border-border/40 mt-6"
      >
      ...
      {sortedElectionRows.length > 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }} className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <InsightCard
            title="Post-Independence Scope"
            text={`Tracking ${sortedElectionRows.length} assembly cycles from ${firstTracked?.year} to ${latestTracked?.year}.`}
          />
          <InsightCard
            title="Leadership Continuity"
            text={longestServingCM ? `${longestServingCM.name} leads tenure charts with ${longestServingCM.total_years.toFixed(1)} years in office.` : 'CM tenure summary is loading.'}
          />
          <InsightCard
            title="Administrative Gap"
            text={presidentsRule ? `President's Rule periods add up to ${presidentsRule.total_years.toFixed(1)} years in the timeline.` : 'No President’s Rule intervals captured.'}
          />
          <InsightCard
            title="Current Track"
            text={currentlyServing ? `${currentlyServing.name} is marked as currently serving in the timeline dataset.` : 'Current serving CM is inferred from open-ended term records.'}
          />
        </motion.div>
      )}
      ...
      {sortedElectionRows.length > 0 && (
        <motion.div
          ...
          className="glass-panel rounded-3xl p-5 border border-border/40 mt-6"
        >
          <h3 className="text-sm font-black uppercase tracking-widest mb-3">Tamil Nadu Assembly Elections Since Independence</h3>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[920px] text-xs">
              <thead>
                <tr className="text-left border-b border-border/40">
                  <th className="py-2 pr-4">Assembly No</th>
                  <th className="py-2 pr-4">Year</th>
                  <th className="py-2 pr-4">State Label</th>
                  <th className="py-2 pr-4">Winner</th>
                  <th className="py-2 pr-4">CM at Start</th>
                  <th className="py-2 pr-4">Seats</th>
                  <th className="py-2 pr-4">Turnout</th>
                </tr>
              </thead>
              <tbody>
                {sortedElectionRows.map((row) => (
                  <tr key={`${row.assembly_no}-${row.year}`} className="border-b border-border/20">
                    <td className="py-2 pr-4 font-semibold">{row.assembly_no}</td>
                    <td className="py-2 pr-4">{row.year}</td>
                    <td className="py-2 pr-4">{row.state_name}</td>
                    <td className="py-2 pr-4">{row.winner}</td>
                    <td className="py-2 pr-4">{row.cm_at_start}</td>
                    <td className="py-2 pr-4">{row.seats_total}</td>
                    <td className="py-2 pr-4">{formatTurnout(row.turnout_percent)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tenureRows.length > 0 && (
        <motion.div
          ...
          className="glass-panel rounded-3xl p-5 border border-border/40 mt-6"
        >
          <h3 className="text-sm font-black uppercase tracking-widest mb-3">Chief Minister Tenure Statistics</h3>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[980px] text-xs">
              <thead>
                <tr className="text-left border-b border-border/40">
                  ...
                  <th className="py-2 pr-4">Total Days</th>
                  <th className="py-2 pr-4">Total Years</th>
                  <th className="py-2 pr-4">First Took Office</th>
                  <th className="py-2 pr-4">Last Day in Office</th>
                  <th className="py-2 pr-4">Acting-Only</th>
                </tr>
              </thead>
              <tbody>
                {tenureRows.map((row) => (
                  <tr key={`${row.name}-${row.party}`} className="border-b border-border/20">
                    <td className="py-2 pr-4 font-semibold">{row.name}</td>
                    <td className="py-2 pr-4">{row.party}</td>
                    <td className="py-2 pr-4">{row.terms}</td>
                    <td className="py-2 pr-4">{row.total_days}</td>
                    <td className="py-2 pr-4">{row.total_years.toFixed(2)}</td>
                    <td className="py-2 pr-4">{formatDateText(row.first_start)}</td>
                    <td className="py-2 pr-4">{formatDateText(row.last_end)}</td>
                    <td className="py-2 pr-4">{row.acting_only ? 'Yes' : 'No'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <motion.div
        ...
        className="glass-panel rounded-3xl p-5 border border-border/40 mt-6"
      >
        <h3 className="text-sm font-black uppercase tracking-widest mb-3">Recent Election Cycle Snapshot Table</h3>
        <div className="overflow-x-auto">
...
const ShiftChip = ({ period, from, to }: { period: string; from: string; to: string }) => {
  const flow = CLASS_TRANSITIONS.find((value) => value.period === period && value.from === from && value.to === to);
  return (
